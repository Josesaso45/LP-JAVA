...
public class UsuarioServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (miSession.getAttribute("usuario") != null) autenticacionUsuario = (Usuario) miSession.getAttribute("usuario");
		if (autenticacionUsuario.getUsuarioId() == 0) response.sendRedirect("autenticacion");
		else if ( !("ADMIN".equals(autenticacionUsuario.getRol())) ) response.sendRedirect("inicio"); 
		else {
			String opcion = "";
